...
@IBDesignable class VerticalStepper: UIControl {
    ...
    var bgHighlightColor = UIColor(red: 0/255, green: 0/255, blue: 0/255, alpha: 0.175)
    var lineColor        = UIColor(red: 122/255, green: 122/255, blue: 122/255, alpha: 1.0).cgColor{
        didSet{
...
@IBDesignable class Plus: Minus {
    ...
    override func draw(_ rect: CGRect) {
        ...
        let a = CGPoint(x: rect.midX - x, y: rect.midY - t)
        let b = CGPoint(x: rect.midX + x, y: rect.midY - t)
        let c = CGPoint(x: rect.midX - t, y: rect.midY - x)
        let d = CGPoint(x: rect.midX - t, y: rect.midY + x)
